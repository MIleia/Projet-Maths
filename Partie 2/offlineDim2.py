...
def offlineDim2(tableau, lmax, Lmax):
    # Conversion des dimensions en unités de 0.1 mètre
    nb_cases_l = int(lmax * 10)
    nb_cases_L = int(Lmax * 10)

    # Trier les objets par aire décroissante (longueur x largeur)
    tableau.sort(key=lambda x: x[0] * x[1], reverse=True)
...
start = time.time()
wagons = offlineDim2(tableau, lmax, Lmax)
end = time.time()
